...
var Observer = function Observer(value) {
  this.value = value;
  console.log('对一个对象，生成一个dep实例，用来管理对象的增删', value); // Charph-log
  this.dep = new Dep();
  this.vmCount = 0;
...
function defineReactive(
  ...
  console.log('针对对象的一个key,生成一个dep小管家实例', obj, key); // Charph-log
  var dep = new Dep();
...
function initMixin(Vue) {
  Vue.prototype._init = function (options) {
    ...
    vm._isVue = true;
    // merge options
    console.log('step1: 将用户传来的options和vue内部内置的options进行合并'); // Charph-log
...
function add$1(event, handler, once$$1, capture, passive) {
  ...
  // * 这里是event定义的地方，可以看到，target是当前的真实dom元素，所以，vue并没有使用事件委托机制
